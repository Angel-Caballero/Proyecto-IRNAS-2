...
BEGIN
  ...
  IF (NUM_UNI > 0) THEN
    RAISE_APPLICATION_ERROR(-20600,'No se puede quitar el recurso porque todavía le quedan unidades');
  ELSE
...
BEGIN
    DELETE FROM ALMACEN WHERE NOMBRE = NOMBRE_A_MOD;
...
BEGIN
    DELETE FROM EQUIPOS_FRIO WHERE ID_FR = NOMBRE_A_MOD;
END;
/
...
(NOMBRE_A_MOD IN USUARIOS.ID_US%TYPE) IS
BEGIN
    DELETE FROM USUARIOS WHERE ID_US = NOMBRE_A_MOD;
END;
/